...
public class Cutter : MonoBehaviour
{
    ...
    {
        // Firstly, calculate the center position by summing up all the vertices and calculating the average
        ...
        // Determine the upward axis using the plane normal
        Vector3 up = _plane.normal;

        // Calculate the left vector perpendicular to the plane normal
        ...
        for (int i = 0; i < _vertices.Count; i++)
        {
            // Calculate displacement and UV coordinates for the first vertex of the triangle
            displacement = _vertices[i] - centerPosition;
            uv1 = new Vector2(
                .5f + Vector3.Dot(displacement, left),
                .5f + Vector3.Dot(displacement, up)
            );

            // Calculate displacement and UV coordinates for the second vertex of the triangle
            ...
            // Create arrays for the vertices, normals, and UVs of the triangle
            ...
            // Check if the mirrored triangle needs to be flipped based on the winding order
            if (Vector3.Dot(Vector3.Cross(vertices[1] - vertices[0], vertices[2] - vertices[0]), normals[0]) < 0)
            {
                FlipTriangle(currentTriangle);
            }

            // Add the mirrored triangle to the right mesh
    ...
    {
        // Flip the winding order of the vertices
        Vector3 tempVertex = triangle.Vertices[1];
        triangle.Vertices[1] = triangle.Vertices[2];
        triangle.Vertices[2] = tempVertex;

        // Flip the winding order of the normals
        Vector3 tempNormal = triangle.Normals[1];
        triangle.Normals[1] = triangle.Normals[2];
        triangle.Normals[2] = tempNormal;

        // Flip the winding order of the UVs
